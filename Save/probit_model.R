...
datalength <- 50000
cov_x<-solve(matrix(rWishart(1,d-1,diag(d-1)),d-1))
#/sample covariates
X <- cbind(rep(1,datalength),rmnorm(datalength, mean_x, cov_x))
#/true parameter value
theta_star <- 1/1:d
#/sample response variables
Y <- rbinom(datalength, size=1, prob=pnorm(X %*%theta_star))
...
stud2_YN<-stud_YN
scale_t<-sqrt(0.1)
h_seq[stud2_YN==1]<-scale_t*h_seq[stud2_YN==1]
...
target_parameters <- list(M=5, prior_mean = rep(0,d),
...
tuning_parameters <- list(B=5, N=1000, K=10000, resampling=SSP_Resampling) 
...
t_in<-20
start<-init_probit(tuning_parameters, target, observations, t_in)


k<-1
hist(start[,k])
abline(v=theta_star[k])
theta_star[k]
...
set.seed(30485)
start_time <- Sys.time()
theta<- online_probit(tuning_parameters, target, observations[1:T_end,], cl=1, particles_in=start, t_in=t_in)
end_time <- Sys.time()
##running time is
