...
T_end<-50000#10^6 #datalength
 set.seed(30485)
start_time <- Sys.time()
theta<- online(tuning_parameters, target2   , observations[1:T_end,], cl=numCores-2)
end_time <- Sys.time()
##running time is
print(end_time-start_time)

tuning_parameters <- list(N=1000, K=10000, J=20,  ESS_bound=0.7, resampling=SSP_Resampling) 
T_end<-10^4
set.seed(30485)
start_time <- Sys.time()
theta<-Online_censored(tuning_parameters, target , observations[1:T_end,], cl=numCores, seed=sample(1:10^5,1))
end_time <- Sys.time()
##running time is
print(end_time-start_time)
...
res1<-matrix(0,2,10^5)
res2<-matrix(0,2,10^5)
for(m in 1:M)
   theta<-Online_censored(tuning_parameters, target , observations[1:T_end,], cl=numCores, seed=sample(1:10^5,1))
