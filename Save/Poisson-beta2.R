...
N_mean<-10^3
N<-rpois(datalength,N_mean)
R<-rbeta(datalength, theta_star[1],theta_star[2])
Y<-rbinom(datalength,size=1,prob=1-exp(-N*R))
observations<-cbind(Y,N)
rm(N,R,Y)
...
target_parameters <- list(sd=1, alpha_val=c(c,alpha), M=100, 
                         prior=c(1/2,1/2),
                          h2_t=h_seq^2,T_p=pert_times, student=stud2_YN, Sigma=Sigma_t,
                          nu=50)
                          
target_parameters$scale_M<-target_parameters$M*gap^2
...
tuning_parameters <- list(N=000, K=10000, ESS_bound=0.7, resampling=SSP_Resampling, J=100) 
...
theta<-online(tuning_parameters, target2,  observations[1:T_end,])
end_time <- Sys.time()
##running time is
