...
online<-function(tuning_parameters, target, observations, cl=1, PI_BAR=FALSE,  t_bar=1){
  #//Input parameters
...
IBIS<-function(tuning_parameters, target, observations, cl=1){
  ...
  for(t in 1:datalength){
    ESS_vec[t]<-ESS
    if(ESS<ESS_bound && t>1){
       Res_times<-c(Res_times,t-1)
       A<-tuning_parameters$resampling(W) 
       Cov_mat<-scale*cov.wt(particles, W)$cov 
       Cov_mat[lower.tri(Cov_mat)] = t(Cov_mat)[lower.tri(Cov_mat)]
       if(min(eigen(Cov_mat)$values)<=10^{-7})  Cov_mat<-diag(diag(Cov_mat),d)
       proposals<-as.matrix(particles[A,])+rmnorm(N, rep(0,d),Cov_mat)
       lik1<-target$loglikelihood(proposals, matrix(observations[1:(t-1),], ncol=data_col),target$parameters, cl)
       lik1[is.nan(lik1)]<--Inf
       alpha<-lik1-lik0[A]+target$dprior(proposals,target$parameters)-target$dprior(as.matrix(particles[A,]),target$parameters)
       U<-log(runif(N))
       work<- U<=alpha
  ...
  return(list(MEAN=mean_vec,MEAN2=mean2_vec,   TILDE=cbind(particles,W), ESS=ESS_vec, ACC=acc_rate, TIMES=Res_times))
...
online_MMD<-function(t_seq, tuning_parameters, target, observations, cl=1, PI_BAR=FALSE,  t_bar=1){
  #//Input parameters
  N<- tuning_parameters$N
  J<- tuning_parameters$J
  K<- tuning_parameters$K
  ...
  save_tilde<-array(0,dim=c(length(t_seq),N,d+1))
  save_bar<-array(0,dim=c(length(t_seq),N,d))
  count<-1
  for(t in 1:datalength){
    ESS_vec[t]<-ESS
    if(ESS<ESS_bound && t>1){
    ...
    if(t==t_seq[count]){
         save_tilde[count,,]<-cbind(particles,W)
         save_bar[count,,]<-particles_bar
         count=count+1
    }   
  }
  return(list(MEAN=mean_vec,MEAN2=mean2_vec, BAR=save_bar, TILDE=save_tilde, ESS=ESS_vec, ACC=acc_rate))
  ...
  for(t in (t_in+1):datalength){
    ...
    if(ESS<ESS_bound && t>1){
       ...
       if(t-1==T_p[p] ||(t-1==t_in && t_in>0) ){  ##h_{t-1}>0
         if(t>T_p[Sig_par[1]]){
            #target$target_parameters$Sigma<-Sigma2/eigen(Sigma2)$values[1]+diag(Sig_par[2],d)
