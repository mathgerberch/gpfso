...
##exponnetial regression model theta_1exp(X_1theta2)+....
##https://www.sciencedirect.com/topics/computer-science/nonlinear-regression-model
rm(list=ls())
...
link<-function(X, theta_star){
 p1<-exp(t(t(X)*theta_star[1:ncol(X)]))
 p1<-t(t(p1)*theta_star[(ncol(X)+1):(2*ncol(X))])
 return( apply(p1,1,sum))
}
#/sample covariates
dx<-d/2
cov_x<-matrix(rWishart(1,dx,diag(dx)),dx)
cov_x<-solve(cov_x)
#cov_x<-diag(1,d-1)
#/sample covariates
X <- rmnorm(datalength, rep(-10, dx) , cov_x)
#/true parameter value
theta_star <-  rnorm(d,mean=3)/d
...
##SGD
############################################################
SGD<-function(observations,c,alpha, theta0){
     n_s<-nrow(observations)
     nc<-ncol(observations)
     d<-2*(nc-1)
     dx<-d/2
     theta<-matrix(0,n_s+1,d)
     theta[1,]<-theta0
     g<-rep(0,d)
     for(i in 1:n_s){
         work<-exp(observations[i,2:nc]*theta[i,1:dx])
         grad<- -c(theta[i,(dx+1):d]*observations[i,2:nc]*work,work)
         if(observations[i,1]< sum(theta[i,(dx+1):d]*work)){
             grad<- -grad
         }  
         theta[i+1,]<-theta[i,]-(c*i^{-alpha})*grad 
     }
     return(theta[2:(n_s+1),])
}

T_end<-10^5
est<-SGD(observations[1:T_end,],c=0.005,alpha=0.5, theta0=runif(d,min=-0.5,max=0.5))
est_bar<-apply(est,2,cumsum)/(1:T_end)
error<-apply(t(abs(theta_star-t(est_bar))),1,max)
...
pert_times<-c(1:start,  datalength)
...
target <- list(dimension = d,
               ...
               loglikelihood =loglikelihood_NLquantile, 
...
#Set tuning and target parameters Use 50000
tuning_parameters <- list(N=500, K=10000,  ESS_bound=0.7, resampling=SSP_Resampling) 
...
set.seed(30485)
start_time <- Sys.time()
theta2<- online(tuning_parameters, target , observations[1:T_end,], cl=numCores-2)
end_time <- Sys.time()
##running time is
...
T_end<-20000
start_time <- Sys.time()
theta<-Online_nonlinear(tuning_parameters, target , observations[1:T_end,], cl=numCores-2, seed=sample(1:10^5,1))
end_time <- Sys.time()
##running time is
print(end_time-start_time)
...
#mean of tilde{pi}
plot(theta2$MEAN[,k],type='l')
abline(h=theta_star[k])


temp<-seq(0,1,length.out=4001)
temp<-temp[2:length(temp)]
temp<-c(temp,rep(1,T_end-length(temp)+10))
