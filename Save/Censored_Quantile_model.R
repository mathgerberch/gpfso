...
#theta_star <- c(-2, rnorm(d-1,mean=0)) ##Difficult: 75%, d=20, Gap=start=15000
...
tuning_parameters <- list(N=6000, K=10000, J=10,  ESS_bound=0.7, resampling=SSP_Resampling) 
...
# set.seed(30485)
start_time <- Sys.time()
theta<- jiter_censored(tuning_parameters, target  , observations[1:T_end,], cl=numCores-2,seed=sample(1:10^5,1))
end_time <- Sys.time()
##running time is
...
tuning_parameters <- list(N=30000, K=30000, J=20,  ESS_bound=0.7, resampling=SSP_Resampling) 
...
# set.seed(30485)
start_time <- Sys.time()
theta<- online(tuning_parameters, target  , observations[1:T_end,], PI_BAR=TRUE,cl=1)#numCores-2)
end_time <- Sys.time()
##running time is
print(end_time-start_time)

tuning_parameters <- list(N=1000, K=30000, J=20,  ESS_bound=0.7, resampling=SSP_Resampling) 
T_end<-10000
#set.seed(30485)
start_time <- Sys.time()
theta<-Online_censored(tuning_parameters, target , observations[1:T_end,], cl=numCores-2, seed=sample(1:10^5,1))
end_time <- Sys.time()
##running time is
print(end_time-start_time)
...
error1<-apply(res1,2,mean)
error2<-apply(res2,2,mean)
...
plot(log(tr), log(error), type='l')
lines(log(tr), log(work),type='l', col='red')


#write.table(error1,"Data/CQR/errorE_d5_q5_M10.txt",col.names=FALSE, row.names=FALSE)
#write.table(error2,"Data/CQR/errorS_d5_q.txt",col.names=FALSE, row.names=FALSE)

#write.table(tr,"Data/CQR/tr_d5_q99.txt",col.names=FALSE, row.names=FALSE)
#write.table(error1,"Data/CQR/errorE_d5_q99.txt",col.names=FALSE, row.names=FALSE)
#write.table(error2,"Data/CQR/errorS_d5_q99.txt",col.names=FALSE, row.names=FALSE)
...
#write.table(theta_star,"Data/CQR/thetaStar_d5_q5_jit.txt",col.names=FALSE, row.names=FALSE)
#write.table(error[points],"Data/CQR/errorS_d5_q5_jit.txt",col.names=FALSE, row.names=FALSE)
#write.table(error2,"Data/CQR/errorS_d5_q99.txt",col.names=FALSE, row.names=FALSE)
